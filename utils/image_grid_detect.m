...
function [gpoints_original, Rnormals] = fisheye_grid_detect(img, cam, numgrid)
    ...
    % Orthonormal mapping to improve grid detection
    %   -> User guided selection of 4 outermost points
    %-----------------------------------------------------------
    mapvars = checkerboard_clickborder(cam, mapvars, ah, false);
    mapvars = get_orthonormal_mapping(mapvars, cam, ah);
    mapvars = map_wide2pers_normal_bestfit(img, cam, mapvars, ah);
    mapvars = checkerboard_clickborder(cam, mapvars, ah, false);
    mapvars = get_orthonormal_mapping(mapvars, cam, ah);
    mapvars = map_wide2pers_normal_bestfit(img, cam, mapvars, ah);
    gpoints = checkerboard_clickborder(cam, mapvars, ah, true);
    ...
    gpoints_original = remap_grid(gpoints, cam, mapvars);
